...
/**
 * Can downgrade a block in a certain range on a triggered event.
 * <triggered_effect trigger="" action="DowngradeBlockInRange, Mods" block_name ="" range="1,1,1" />
 */

public class MinEventActionDowngradeBlockInRange : MinEventActionTargetedBase
{

    /**
     * Executing the downgrades
     */
    ...
    public override void Execute(MinEventParams _params)
    {
        ...
        for (int i = 0; i < this.targets.Count; i++)
        {
            ...
            foreach (KeyValuePair<Vector3i, Block> entry in blockPositions)
            {
                ...
                BlockHelpers.DoBlockDowngrade(entry.Key);
            }
        }
    }
    ...
    public override bool ParseXmlAttribute(XmlAttribute _attribute)
    {
        ...
        if (!flag)
        {
            ...
            {
                bool playSound;
                if (StringParsers.TryParseBool(_attribute.Value, out playSound))
                {
                    return false;
                }
